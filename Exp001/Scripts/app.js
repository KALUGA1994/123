...
app.controller('AdrGet', function ($scope, $http, Get) {
    $scope.pagingInfo = {
        itemsPerPage: "10",
        sortBy: null,
        reverse: false,
    ...
    $scope.getRoles = function () {
        $http.get('GetUsers/getRoles/').then(function (data) {
            $scope.roles = data.data;
        })
    }
    // получение статусов
    $scope.getStatus = function () {
        $http.get('GetUsers/getStatuses/').then(function (data) {
    ...
    function get() {
        Get.GetAdres($scope.pagingInfo).then(function (data) {
            angular.forEach(data.data.data, function (a, key) {
                data.data.data[key].dataRozh = moment(data.data.data[key].dataRozh).format('DD.MM.YYYY');
            });
            $scope.adr = data.data;
